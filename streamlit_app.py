...
def home():
    ...
    col3.metric(label  ="Pico de consumo nas últimas 24 horas: ", value=f"{coleta_dados_csv()[opção_regiao].iloc[-24:-1].max()} MWh", 
                ...
                help = f"Valor do consumo de energia ás {pd.to_datetime(coleta_dados_csv()['Datetime']).iloc[coleta_dados_csv().columns.get_loc(coleta_dados_csv()[opção_regiao].iloc[-24:-1].max())]}")
